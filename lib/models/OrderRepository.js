...
var OrderRepository = function () {
  ...
  function OrderRepository(orders, sheets, casesIndex, displays) {
  ...
  _createClass(OrderRepository, [{
    key: 'findCase',
    value: function findCase(productName, size) {
    ...
    value: function patchOrder(orderId, newOrderAttributes) {
      if (this.orders[orderId]) {
        ...
        _logger2.default.info('new order: ', newOrder);
    ...
    value: function addOrderToSheet(order, sheet) {
      ...
      order.invoiceNumber = order.invoiceNumber || (0, _sprintfJs.sprintf)('BFD%06d', Object.keys(this.orders).length + 1349);
      ...
      return setHeaderRow(columnHeaders).then(function () {
        ...
        var _loop = function _loop() {
          ...
          var commission = order.commissions[i];
    ...
    key: 'getCommissionFromRow',
    value: function getCommissionFromRow(row) {
    ...
    value: function createFromSheets(sheets, cases, displays) {
      /*
       * Loop through all of the sheets in the workbook
       */
      var casesIndex = (0, _lodash2.default)(cases).groupBy(function (caseInfo) {
      ...
      sheets.forEach(function (sheet) {
        ...
        sheetPromises.push(getRows({
          ...
          rows.forEach(function (row) {
            ...
            var commission = OrderRepository.getCommissionFromRow(row);
      ...
      return _bluebird2.default.all(sheetPromises).then(function () {
        return new OrderRepository(orders, sheetIndex, casesIndex, displays);
      });
    }
  }]);
