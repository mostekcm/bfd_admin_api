...
var OrderService = function () {
  function OrderService() {
    ...
    this.doc = new _googleSpreadsheet2.default((0, _config2.default)('ORDERS_SHEET'));
  ...
  _createClass(OrderService, [{
      ...
      return this.getInfo().then(function (info) {
        ...
        return caseService.getAll().then(function (cases) {
          return displayService.getAll().then(function (displays) {
            return _OrderRepository2.default.createFromSheets(info.worksheets, cases, displays).then(function (repo) {
    ...
    key: 'putOrder',
    value: function putOrder(orderId, newOrder) {
      return this.getOrderRepo().then(function (repo) {
        return repo.putOrder(orderId, newOrder);
      });
    }
  }, {
    key: 'deleteOrder',
    ...
    value: function getNextMonthOrders() {
      return this.getOrderRepo().then(function (repo) {
        ...
        return _lodash2.default.filter(orders, function (order) {
          return order.targetShipDate && order.targetShipDate < endTime && !order.shippedDate;
        });
      });
    }
  }, {
    ...
    value: function getMonthOrders(month, year) {
      return this.getOrderRepo().then(function (repo) {
        ...
        return _lodash2.default.filter(orders, function (order) {
          return order.targetShipDate && order.targetShipDate >= startTime && order.targetShipDate < endTime && !order.shippedDate;
        });
      });
    }
  }, {
    ...
    value: function getPendingCommissionOrders() {
      return this.getOrderRepo().then(function (repo) {
        ...
        return _lodash2.default.filter(orders, function (order) {
          return (!order.commissionPaidDate || !order.jesPaidDate) && order.payments.length > 0;
        });
      });
    }
  }, {
    key: 'getAllNotCancelled',
    value: function getAllNotCancelled() {
      return this.getOrderRepo().then(function (repo) {
        var orders = repo.getAll();
